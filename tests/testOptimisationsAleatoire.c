...
void InitJeu() {
	...
	PlateauCourant.TempsRestant = 30000;
	startTimer();
	timerRunning = true;
}
...
int main(int argc, char **argv)
{
    ...
    PlateauCourant = fichierVersPlateau(DATA_DIRECTORY "/pg29.txt");
    ...
    for(i = 0; i < (&PlateauCourant) -> nbCartes; i++) {
        Carte* carte = &((&PlateauCourant) -> pioche[i]);
		...
		for(j = 0; j < carte -> nbIcones; j++) {
			...
			float distance = rand() % CARD_RADIUS + (CARD_RADIUS / 2);
			float angle = 2 * M_PI * j / (carte -> nbIcones);

			ic -> position = newVect2(distance * cos(angle),\
										distance * sin(angle));
		}
		...
		while(!(carteIsValid(carte) && compteur > 100)) {
			...
			*/
			...
			updateIcones(carte);
			(&PlateauCourant) -> haut = *carte;
			(&PlateauCourant) -> bas = *carte;
			renderSceneJeu();
			usleep(100);
			compteur++;
		}	
...
// TODO : 
// - Polish
// - Optimisation placement aléatoire
// - d'autres icones
// - mode de difficultés
// - highscores